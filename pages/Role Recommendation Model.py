...
def extract_keywords(text, min_length=4):
    ...
    stopwords = {'the', 'and', 'to', 'of', 'a', 'in', 'for', 'is', 'on', 'that', 'with', 'as', 'an'}
...
def calculate_ats_score(resume_text, job_description):
    ...
    return ats_score, matched_keywords, [k for k in job_keywords if k not in matched_keywords]
...
def analyze_resume_job_fit(resume_text, job_description):
    ...
    try:
        response = generate_content_with_llm(prompt, model=st.session_state.get("llm_model"))
...
def main():
    ...
    with tabs[0]:  # Resume Upload Tab
        ...
        uploaded_file = st.file_uploader("Upload your resume (PDF, DOCX, TXT)", type=["pdf", "docx", "txt"], key="resume_upload_tab")
        ...
        if uploaded_file:
            ...
            if resume_text:
                ...
                with st.expander("View Extracted Resume Text"):
                    st.markdown(f"```{resume_text[:1000]}{'...' if len(resume_text) > 1000 else ''}```")
                    ...
                    with st.spinner("Finding matching roles..."):
                        ...
                        for i, (job_title, score) in enumerate(recommended_jobs):
                            ...
                            with st.expander(f"Salary Estimate for {job_title}"):
                                salary_estimate = generate_salary_estimate(job_title, skills)
            ...
            if job_description.strip():
                if st.button("Analyze Job Fit"):
                    with st.spinner("Analyzing job fit..."):
                        ...
                        st.success(f"Overall Job Compatibility Score: {similarity_score:.1f}%")
                ...
                if "job_fit_metrics" in st.session_state:
                    ...
                    with st.expander("Learning Resources for Missing Skills"):
                        if st.session_state.missing_keywords:
                            resources = generate_learning_resources(st.session_state.missing_keywords)
                            st.write(resources)
                        else:
                    ...
                    with st.expander("Interview Preparation"):
                        if st.button("Prepare for Interview"):
                            ...
                            create_docx_download_button(interview_prep, "interview_prep.docx")
